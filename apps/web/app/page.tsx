...
export default function HomePage() {
  ...
  return <main className="brand-site">
    ...
    <section id="inspiracao" className="inspiration-section"><div className="inspiration-heading"><span className="brand-symbol large">M</span><span className="green-kicker">Inspirações em movimento</span><h2>Encontre seu próximo horário.</h2><p>Um espaço visual para novidades, serviços e conteúdos que ajudam seu cliente a escolher.</p></div><div className="inspiration-carousel"><div className="inspiration-scene"><div className="inspiration-ring">{[...gallery, ...gallery, ...gallery].map((item, index) => <article className={`inspiration-panel ${item.tone}`} style={panelStyle(index)} key={`${item.title}-inspiration-${index}`}><span className="panel-mark">{item.title.slice(-1)}</span><span><small>{item.type}</small>{item.title}</span></article>)}</div></div></div></section>
